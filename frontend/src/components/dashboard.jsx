...
  const handleLogout = () => {
    // Simulate a delay for a better user experience (loading state)
    // In a production app, you might be waiting for a server response here.
    setTimeout(() => {
...
      });
      ...
      setIsLoggingOut(false); // Hide the modal (though navigation will cover it)
      navigate('/login');
    }, 500); 
  };
...
                onClick={(e) => {
                  e.preventDefault();
                  setActiveView("settings"); // <--- SET activeView to "settings"
                }}
                style={{
...
              <Link to="/help"><MdOutlineHelp className="icon" />Help & Support</Link>
              {/* UPDATED LOGOUT LINK TO CALL confirmLogout */}
