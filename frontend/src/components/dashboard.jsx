...
//   // Helper function to get user info (same as PetitionHead)
//   const getUserInfo = () => {
...
//         // Store all petitions
//         setAllPetitions(normalizedPetitions);
        
//         // Count total active petitions (all users)
//         const totalActivePetitions = normalizedPetitions.filter(p => 
//           p.status === 'active' || 
...
//         // Filter only user's petitions using the same logic as PetitionHead
//         const userPetitions = normalizedPetitions.filter(p => isUserOwnPetition(p));
        
//         console.log("Total active petitions:", totalActivePetitions);
//         console.log("User petitions count:", userPetitions.length);
        
//         // Set only user's petitions for display
//         setPetitions(userPetitions);
        
//         setUserStats(prev => ({
//           ...prev,
//           totalActivePetitions: totalActivePetitions,
//           myPetitions: userPetitions.length
//         }));
...
//         // Filter active polls (not closed) - all users
//         const today = new Date();
//         const totalActivePolls = fetchedPolls.filter(poll => {
//           if (!poll.closesOn) return true;
//           const closesDate = new Date(poll.closesOn);
//           return closesDate >= today;
//         }).length;
...
//         // Filter user's polls using the same matching logic
//         const userPolls = fetchedPolls.filter(p => {
//           let creatorId;
//           if (typeof p.createdBy === "object" && p.createdBy !== null) {
//             creatorId = p.createdBy._id?.toString() || p.createdBy.toString();
...
//           if (matches) {
//             console.log("Found user poll:", p.title);
//           }
//           return matches;
//         });
        
//         // Filter user's active polls
//         const activeUserPolls = userPolls.filter(poll => {
//           if (!poll.closesOn) return true;
//           const closesDate = new Date(poll.closesOn);
//           return closesDate >= today;
//         });
        
//         console.log("Total active polls:", totalActivePolls);
//         console.log("User polls count:", userPolls.length);
        
//         setPolls(activeUserPolls);
...
//                   onClick={() => setActiveTab("petitions")}
//                 >
//                    Petitions ({userStats.totalActivePetitions})
//                 </button>
//                 <button
//                   className={`tab-btn ${activeTab === "polls" ? "active" : ""}`}
//                   onClick={() => setActiveTab("polls")}
//                 >
//                    Polls ({userStats.totalActivePolls})
//                 </button>
//               </div>

//               <div className="filters-container">
//                 <div className="filter-controls">
//                   <h2>My {activeTab === "petitions" ? "Petitions" : "Polls"}</h2>
//                   <div className="location-filter">
